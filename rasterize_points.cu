	...
	const bool debug)
{
  ...
  torch::Tensor gs_per_pixel = torch::full({20, H, W}, -1.0, float_opts);
  torch::Tensor weight_per_gs_pixel = torch::full({20, H, W}, 0.0, float_opts);
  torch::Tensor x_mu = torch::full({20, 2, H, W}, 0.0, float_opts);
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer, proj_2D, conic_2D, gs_per_pixel, weight_per_gs_pixel, x_mu);
}
